...
class DeportrationStatusResource extends JsonResource
{
    ...
    {
        $order_status = DeportrationStatus::where("deportration_id", $orderId)->where("status_id", $status->id)->get()->first();

        $maid_name = Deportration::find($orderId)->maid->fullName;
    ...
    {
        $order = Deportration::with("statuses")->find(request()->id);
        if (!$order->status_id) {
